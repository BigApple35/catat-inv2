...
import { submitQuizResult } from "@/api/quiz-api";
...
export function QuizComp({ materialId, onAnswer, onComplete }: Props) {
  ...
  const [quizID, setQuizID] = useState()
    ...
    pilihan: q.pilihan.map((p: any) => ({
      [p.key]: p.value,   // convert {key:"A",value:"xxx"} → {A:"xxx"}
    })),
  }))
          setQuestions(mapped);
        } catch (err) {
